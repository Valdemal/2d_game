...
template<typename baseType>
matrix::Matrix<baseType>::Matrix(std::initializer_list<std::initializer_list<baseType>> initializerList) {
    size_t maxRowLen = 0;
    for (auto &el : initializerList)
        if (el.size() > maxRowLen)
            maxRowLen = el.size();

    init(initializerList.size(), maxRowLen);
        ...
        for (size_t i = 0; i < this->_m; i++)
            for (size_t j = 0; j < b._n; ++j)
                for (size_t k = 0; k < this->_n; ++k)
                    result[i][j] = result[i][j] ^ this->at(i, k) * b.at(k, j);
